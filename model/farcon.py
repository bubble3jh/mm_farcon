...
class FarconVAEnoY(nn.Module):
    def __init__(self, args, device):
        super(FarconVAEnoY, self).__init__()
        input_dim = args.n_features + args.s_dim
        if args.encoder == 'mlp':
            self.encoder = Encoder(input_dim, 2 * args.latent_dim, args.hidden_units, args)
            self.decoder = Decoder(2 * args.latent_dim, args.n_features + args.s_dim, args.hidden_units, args)
        else:
    ...
    def encode(self, x, s, input_label=False, return_z=False):
    ...
    def forward(self, x, s, input_label=False):
        (zx, zs), (mu_x, logvar_x, mu_s, logvar_s) = self.encode(x, s, input_label)
        x_recon, s_recon = self.decode(zx, zs)
        return (zx, zs), (x_recon, s_recon), (mu_x, logvar_x, mu_s, logvar_s)
